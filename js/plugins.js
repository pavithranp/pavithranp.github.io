...
(function ($) {
	...
	var start_data_programming = new Date(2015, 0, 0); // month is zero based                       
	var avg_lines_code_day = 50
	var lines_code = avg_lines_code_day * 0.5 * Math.round((now - start_data_programming) / (1000 * 60 * 60 * 24)); 
	
	// approx. coffees
	var start_drinking_coffee = new Date(2009, 0, 0); // month is zero based                        
	var avg_coffee_day = 3
	var num_coffee = avg_coffee_day * Math.round((now - start_drinking_coffee) / (1000 * 60 * 60 * 24)); 
	
	// approx. papers read
	var start_reading_papers = new Date(2016, 0, 0); // month is zero based                        
	var avg_paper_day = 0.07
	var num_papers = Math.round(avg_paper_day * Math.round((now - start_reading_papers) / (1000 * 60 * 60 * 24))); 

	// approx. DNN training hours
	var start_training_ml = new Date(2016, 0, 0); // month is zero based                        
	var avg_ml_training_day = 16
	var ml_training_hours = avg_ml_training_day * Math.round((now - start_training_ml) / (1000 * 60 * 60 * 24)); 
	
	$("#facts").appear(function () {
		$("#number_1").animateNumber({
			number: lines_code	     // Lines of code - Change to your number			
		}, 2200);
		$("#number_2").animateNumber({
			number: num_papers		// papers - Change to your number	
		}, 2200);
		$("#number_3").animateNumber({
			number: num_coffee		// Coffee - Change to your number
		}, 2200);
		$("#number_4").animateNumber({
			number: ml_training_hours	// DNN training hours - Change to your number
		}, 2200);
	}, {
			accX: 0,
			accY: -150
	...
	$("#skills").appear(function () {
		...
		$("#chart_programming_python").animateNumber({
			number: 82		// Python - Change to your number
		}, 1500);
		$("#chart_programming_cpp").animateNumber({
			number: 71		// C++ - Change to your number
		}, 1500);
		$("#chart_programming_java").animateNumber({
			number: 62		// java - Change to your number
		}, 1500);
		$("#chart_programming_matlab").animateNumber({
			number: 56		// matlab - Change to your number
		}, 1500);
		$("#chart_programming_sql").animateNumber({
			number: 38		// sql - Change to your number
		}, 1500);
		$("#chart_programming_html").animateNumber({
			number: 42		// html - Change to your number
		}, 1500);
		$("#chart_other_git").animateNumber({
			number: 73		// git - Change to your number
		}, 1500);
		$("#chart_other_linux").animateNumber({
			number: 67		// linux - Change to your number
		}, 1500);
		$("#chart_specialization_computer_vision").animateNumber({
			number: 86		// computer vision - Change to your number
		}, 1500);
		$("#chart_specialization_ml").animateNumber({
			number: 76		// machine learning - Change to your number
		}, 1500);
		$("#chart_infrastructure_aws").animateNumber({
			number: 41		// AWS - Change to your number
		}, 1500);
	}, {
			accX: 0,
			accY: -150
